...
IntPt = tuple[int, int]  # (x, y) coordinate of a pixel
FloatPt = tuple[float, float]  # (x, y) coordinate of a point
Line = tuple[float, float, float]  # (rho, xn, yn), xn = cos(theta), yn = sin(theta)

IntPtArray = np.ndarray[tuple[int, Literal[2]], int]  # array of (x, y) pixel coordinates
FloatPtArray = np.ndarray[tuple[int, Literal[2]], float]  # array of (x, y) point coordinates
